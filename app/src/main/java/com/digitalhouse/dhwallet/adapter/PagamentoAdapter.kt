...
class PagamentoAdapter(private val items: List<PagamentosFavoritos>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        return PagamentoViewHolder(inflater.inflate(R.layout.item_contact,parent,false))
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder){
            is PagamentoViewHolder -> holder.bind(items[position])
        }
    }
...
class PagamentoViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(item: PagamentosFavoritos){
